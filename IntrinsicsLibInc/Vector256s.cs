        ...
        static Vector256s() {
            ...
            for (i = 0; i < MaskBitPosArray8B.Length; ++i) {
                if (i < MaskBitPosArray1B.Length) {
                    MaskBitPosArray1B[i] = Vector256s.Create(Scalars.GetByBits<Byte>(bitpos));
                    MaskBitsArray1B[1 + i] = Vector256s.Create(Scalars.GetByBits<Byte>(bits));
                }
                if (i < MaskBitPosArray2B.Length) {
                    MaskBitPosArray2B[i] = Vector256.AsByte(Vector256s.Create(Scalars.GetByBits<UInt16>(bitpos)));
                    MaskBitsArray2B[1 + i] = Vector256.AsByte(Vector256s.Create(Scalars.GetByBits<UInt16>(bits)));
                }
                if (i < MaskBitPosArray4B.Length) {
                    MaskBitPosArray4B[i] = Vector256.AsByte(Vector256s.Create(Scalars.GetByBits<UInt32>(bitpos)));
                    MaskBitsArray4B[1 + i] = Vector256.AsByte(Vector256s.Create(Scalars.GetByBits<UInt32>(bits)));
                }
                if (i < MaskBitPosArray8B.Length) {
                    MaskBitPosArray8B[i] = Vector256.AsByte(Vector256s.Create(Scalars.GetByBits<UInt64>(bitpos)));
                    MaskBitsArray8B[1 + i] = Vector256.AsByte(Vector256s.Create(Scalars.GetByBits<UInt64>(bits)));
                }
                // next.
                bitpos <<= 1;
                bits = bits << 1 | 1;
